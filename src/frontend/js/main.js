...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedDoctorId) {
        ...
        appState.doctorId = parseInt(savedDoctorId);
        appState.doctorName = savedDoctorName;
        if(savedDoctorEmail) appState.doctorEmail = savedDoctorEmail;
...
function toggleUrogenital(gender) {
    const obsBox = document.getElementById('cp-int-obs');
    if (gender === 'female') {
...
async function handleCreatePatient(e) {
    ...
    formData.append("urogenital", uroObs); 
    ...
    let finalHeight = parseFloat(document.getElementById('cp-height').value) || 0;
    if (document.getElementById('cp-height-unit').value === 'cm') finalHeight = finalHeight / 100;
    ...
    const physExamBase = document.getElementById('cp-phys-exam').value;
    const imcText = document.getElementById('imc-result').innerText;
    const qtcText = document.getElementById('qtc-result').innerText;
    formData.append("physical_examination", `[${imcText} | ${qtcText}]\n${physExamBase}`);
...
async function loadPatientDetails(document_id) {
    try {
        ...
        if (data.status === 'success') {
            ...
            `;
            if(ant.gender === 'female') antHtml += `<p><strong>Urogenital y Obstétrico:</strong> ${ant.urogenital || '-'}</p>`;
            else antHtml += `<p><strong>Urogenital:</strong> ${ant.urogenital || '-'}</p>`;
            ...
            data.queries.forEach(q => {
                ...
                timeline.innerHTML += `
                    <div style="border-left: 3px solid var(--primary-color); padding-left: 15px; margin-bottom: 15px; background: #f9f9f9; padding: 10px; border-radius: 0 5px 5px 0;">
                        ...
                        <p style="font-size: 13px;"><strong>Tratamiento:</strong> ${q.treatment || '-'}</p>
                        ${examsHtml}
                    </div>`;
            });
    ...
    const printContent = `
        ...
        <head>
            ...
            <style>
                ...
                html, body { background-color: #fff; margin: 0px; padding: 0px; }
                @media print { thead { display: table-header-group; } tfoot { display: table-footer-group; } body { -webkit-print-color-adjust: exact; } }
...
async function handleEvolutionaryQuery(e) {
    ...
    try {
        ...
        if (result.status === 'success') {
            alert(result.msg);
            loadPatientDetails(appState.currentPatientData.document_id); 
        } else {
            alert(`Error: ${result.msg}`);
        }
    } catch (error) { console.error(error); }
...
// --- CALCULATE VITALS (BMI & QTc) ---
function calculateVitals() {
    let weight = parseFloat(document.getElementById('cp-weight').value);
    let height = parseFloat(document.getElementById('cp-height').value);
    const weightUnit = document.getElementById('cp-weight-unit').value;
    const heightUnit = document.getElementById('cp-height-unit').value;

    if (weight && height) {
        let wKg = weightUnit === 'lb' ? weight * 0.453592 : weight;
        let hM = heightUnit === 'cm' ? height / 100 : height;
        const bmi = wKg / (hM * hM);
        ...
        document.getElementById('imc-result').innerText = `IMC: ${bmi.toFixed(2)} ${status}`;
    } else {
        document.getElementById('imc-result').innerText = "IMC: --";
    }

    const hr = parseFloat(document.getElementById('cp-hr').value);
    const qt = parseFloat(document.getElementById('cp-qt').value);

    if (hr > 0 && qt > 0) {
        const rr = 60 / hr; 
        const qtc = qt / Math.sqrt(rr);
        
        const gender = document.getElementById('cp-pat-gender').value;
        const limit = gender === 'female' ? 460 : 440;
        const qtcStatus = qtc > limit ? "⚠️ (Prolongado)" : "✅ (Normal)";
        
        document.getElementById('qtc-result').innerText = `QTc (Bazett): ${qtc.toFixed(0)} ms ${qtcStatus}`;
    } else {
        document.getElementById('qtc-result').innerText = "QTc (Bazett): -- ms";
    }
}